...
# Reflect on Project Diary Entries and Synthesize Insights

You are going to analyze multiple diary entries from the **current project** to identify recurring patterns, synthesize insights, and propose updates to the project's CLAUDE.md file.

## Important: Project-Only Focus

**Only analyze diary entries from the current project's `.claude/memory/diary` directory:**
- Diary entries located in `{{ cwd }}/.claude/memory/diary/`
- This ensures reflections are project-specific and relevant
- Patterns and insights will be contextual to this project's needs and workflows

**Exclude:**
- Global diary entries from `~/.claude/memory/diary/`
- Entries from other projects
- Non-project-related activities and preferences
...
- **Include processed**: "include all entries" to re-analyze already processed entries

If no parameters are provided, default to analyzing the **last 10 unprocessed diary entries** from the current project.

## Steps to Follow

1. **Check processed entries log**:
   - Read `{{ cwd }}/.claude/memory/reflections/processed.log` to find already-processed diary entries
   ...
   - Create the file if it doesn't exist (see cross-platform script below)

2. **Locate project diary entries**:
   - Directory: `{{ cwd }}/.claude/memory/diary/` (project-local diary entries)
   ...
   - **Only include entries for the current project**
...
4. **Read and parse filtered diary entries**:
   - Read each diary entry file from the project directory
   - Extract information from all sections
   - Pay special attention to project-specific sections:
     - Project Preferences & Standards
     - Project Design Decisions
     - Project Challenges & Solutions
     - Project Patterns

5. **Create the project reflections directory** (if it doesn't exist):
   - Directory: `{{ cwd }}/.claude/memory/reflections/`
   - Use cross-platform script to create it automatically

6. **Read current CLAUDE.md files**:
   - Read `~/.claude/CLAUDE.md` for global rules
   - Read `{{ cwd }}/.claude/CLAUDE.md` for project-specific rules (if it exists)
...
Below are proposed additions to your project's `{{ cwd }}/.claude/CLAUDE.md` file. **Review these carefully before adding them.**

### Section: [Project-Specific / Code Quality / Git Workflow / Testing / etc.]
...
**Note**: These rules are project-specific and will be saved to `{{ cwd }}/.claude/CLAUDE.md`. Global rules should be added to `~/.claude/CLAUDE.md`.
...
9. **Save the reflection document**:
   - Use the appropriate cross-platform script to create directories and save the reflection
   - Filename format: `YYYY-MM-reflection-N.md` (increment N if multiple reflections in same month)
   - Save to: `{{ cwd }}/.claude/memory/reflections/[filename]`
...
10. **Automatically update CLAUDE.md**:
   ...
   **PRIORITY 2: Add new rules to project CLAUDE.md**
   - Create or update `{{ cwd }}/.claude/CLAUDE.md` for project-specific rules
...
11. **Update processed entries log**:
   - Append processed diary entries to `{{ cwd }}/.claude/memory/reflections/processed.log`
...
### Create Project Reflections Directory

#### For Unix/Linux/macOS:
```bash
PROJECT_ROOT="{{ cwd }}" && \
mkdir -p "$PROJECT_ROOT/.claude/memory/reflections" && \
# Create processed.log if it doesn't exist
touch "$PROJECT_ROOT/.claude/memory/reflections/processed.log" && \
echo "Created reflections directory: $PROJECT_ROOT/.claude/memory/reflections"
```

#### For Windows (PowerShell):
```powershell
$projectRoot = "{{ cwd }}"
New-Item -ItemType Directory -Force -Path "$projectRoot\.claude\memory\reflections" | Out-Null
$processedLog = "$projectRoot\.claude\memory\reflections\processed.log"
if (!(Test-Path $processedLog)) {
    New-Item -ItemType File -Path $processedLog | Out-Null
}
Write-Output "Created reflections directory: $projectRoot\.claude\memory\reflections"
```
...
if not exist "%PROJECT_ROOT%\.claude\memory\reflections" mkdir "%PROJECT_ROOT%\.claude\memory\reflections"
if not exist "%PROJECT_ROOT%\.claude\memory\reflections\processed.log" echo. > "%PROJECT_ROOT%\.claude\memory\reflections\processed.log"
echo Created reflections directory: %PROJECT_ROOT%\.claude\memory\reflections
```

### Generate Reflection Filename
...
TODAY=$(date +%Y-%m) && \
N=1 && \
while [ -f "$PROJECT_ROOT/.claude/memory/reflections/${TODAY}-reflection-${N}.md" ]; do N=$((N+1)); done && \
REFLECTION_FILE="$PROJECT_ROOT/.claude/memory/reflections/${TODAY}-reflection-${N}.md" && \
echo "Reflection file path: $REFLECTION_FILE"
```
...
$n = 1
$reflectionPath = "$projectRoot\.claude\memory\reflections"
$reflectionFile = "$reflectionPath\${today}-reflection-${n}.md"

while (Test-Path $reflectionFile) {
    $n++
    $reflectionFile = "$reflectionPath\${today}-reflection-${n}.md"
}

Write-Output "Reflection file path: $reflectionFile"
```
...
@echo off
set PROJECT_ROOT={{ cwd }}
for /f "tokens=*" %%i in ('powershell -Command "Get-Date -Format ''yyyy-MM''"') do set TODAY=%%i
set N=1
:check_reflection
if exist "%PROJECT_ROOT%\.claude\memory\reflections\%TODAY%-reflection-%N%.md" (
    set /a N+=1
    goto check_reflection
)
set REFLECTION_FILE=%PROJECT_ROOT%\.claude\memory\reflections\%TODAY%-reflection-%N%.md
echo Reflection file path: %REFLECTION_FILE%
```
...
- "Always use TypeScript strict mode" (appears in 5 sessions across this project)
- "Prefer functional components in React" (appears in 4 sessions in this React project)
...
- If no diary entries exist in the project directory, inform the user and suggest running `/diary` first
- Check both `{{ cwd }}/.claude/memory/diary/` for diary entries and report what was found
- If the project reflections directory cannot be created, report the specific path and permissions error
...
**Default behavior**: Skip entries already listed in `{{ cwd }}/.claude/memory/reflections/processed.log`
...
```
# Analyze last 10 unprocessed entries from current project (default)
/reflect

# Analyze last 20 unprocessed entries from current project
...
# Analyze last 15 entries related to React
...
**Important**: The `.claude/memory` directory should already be in your project's `.gitignore` file (as configured by the diary command). This ensures:
- Project-specific reflections remain local to each developer
- Sensitive project patterns and insights aren't accidentally committed
- The reflection serves as personal learning for the developer's work on the project

If `.claude/memory/` is not in your `.gitignore`, add this line:
```
.claude/memory/
```
